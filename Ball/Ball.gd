...
func _on_Ball_body_entered(body):
	if body.has_method("hit"):
		body.hit(self)
		accelerate = true
		$Tween.interpolate_property($Images/Highlight, "modulate:a", 0.6, 0.0, time_highlight, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
		$Tween.interpolate_property($Images/Highlight, "scale", Vector2(0.1, 0.1), Vector2(0.8, 0.8), time_highlight_size, Tween.TRANS_BOUNCE, Tween.EASE_IN)
		$Tween.start()
...
func distort():
	var dir = Vector2(1 + linear_velocity.length() * distort_effect, 1 - linear_velocity.length() * distort_effect)
